...
- Токен-аутентификация (`X-Auth`), брутфорс-защита: 5 промахов → IP в блок на 10 мин
- Сессия админа **бессрочная**: `POST /api/login` ставит cookie на 10 лет
- `web_admins` — список админов со своими токенами, сервер помнит, кто вошёл
  (`/api/whoami`), действия пишутся в аудит с именем исполнителя
- Security-заголовки (XFO/nosniff/CSP), анти-CSRF (POST только честным JSON + X-Auth)
- Секреты в config.json шифруются через **Windows DPAPI** (`dpapi:...`) —
  скопированный файл на другой машине секреты не раскрывает
- HTTPS (самоподписанный): `tools\make_cert.ps1`, затем `"web_tls": {"enabled": true}`
- Отчёты и `/metrics` под auth; `web_auth_enabled` через API отключить нельзя
